...
class NofoExportPolicyLanguageRenderingTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.altered_slot = PolicyLanguageSlot.objects.get(slot_key="DG-016", is_current=True)
        cls.dg018_slot = PolicyLanguageSlot.objects.get(slot_key="DG-018", is_current=True)
        ...
        Subsection.objects.create(
            section=section, name=cls.dg018_slot.name, tag="h4",
            body="A modified version of the funding preferences language.", order=3,
            policy_language_status="may_be_altered", policy_language_slot=cls.dg018_slot,
        )
        Subsection.objects.create(
    ...
    @override_settings(HHS_NOFO_POLICY_EXPORT_ENABLED=True)
    def test_flag_on_stripped_export_elevates_dg018(self):
        resp = self.client.get(self.export_url + "?policy_stripped=1")
        body = resp.content.decode()
